-- [[ FH4N HUB - V.490 FIX TOTAL SEMUA FITUR ]] --
...
local Colors = {
    ["Merah"]=Color3.fromRGB(255,0,0), ["Kuning"]=Color3.fromRGB(255,255,0), ["Biru"]=Color3.fromRGB(0,0,255),
    ["Hijau"]=Color3.fromRGB(0,255,0), ["Ungu"]=Color3.fromRGB(170,0,255), ["Hitam"]=Color3.fromRGB(0,0,0),
    ["Putih"]=Color3.fromRGB(255,255,255), ["Cyan"]=Color3.fromRGB(0,255,255), ["Lime"]=Color3.fromRGB(191,255,0)
}

-- --- UI RENDERER ---
local target = (game:GetService("CoreGui") or Player:WaitForChild("PlayerGui"))
if target:FindFirstChild("FH4N_V490") then target["FH4N_V490"]:Destroy() end
local sg = Instance.new("ScreenGui", target); sg.Name = "FH4N_V490"
...
local Main = Instance.new("Frame", sg); Main.Size = UDim2.new(0, 580, 0, 520); Main.Position = UDim2.new(0.5, -290, 0.5, -260); Main.BackgroundColor3 = _G.FH4N_V490.BGColor; Main.Active = true; Drag(Main)
Instance.new("UICorner", Main).CornerRadius = UDim.new(0, 15); local Stroke = Instance.new("UIStroke", Main); Stroke.Thickness = 2; Stroke.Color = _G.FH4N_V490.StrokeColor

local TitleH = Instance.new("TextLabel", Main); TitleH.Size = UDim2.new(1, 0, 0, 50); TitleH.Text = "FH4NHUB V.490 FINAL FIX"; TitleH.TextColor3 = Color3.new(1,1,1); TitleH.Font = "GothamBold"; TitleH.TextSize = 22; TitleH.BackgroundTransparency = 1
...
local function Toggle(p, txt, key)
    ...
    b.MouseButton1Click:Connect(function() _G.FH4N_V490[key] = not _G.FH4N_V490[key]; b.Text = "  "..txt..": "..(_G.FH4N_V490[key] and "ON" or "OFF"); b.TextColor3 = _G.FH4N_V490[key] and Color3.new(0,1,0.5) or Color3.new(1,1,1) end)
end
local function Input(p, pl, key)
    ...
    i.FocusLost:Connect(function() _G.FH4N_V490[key] = tonumber(i.Text) or i.Text end)
end

-- --- PAGE 1: PLAYER ---
Toggle(pg1, "God Mode", "GodMode"); Toggle(pg1, "ESP Player", "Esp"); Toggle(pg1, "Fly Mode", "Fly"); Toggle(pg1, "Noclip", "Nc"); Toggle(pg1, "Inf Jump", "InfJump")
Input(pg1, "WalkSpeed", "WS"); Input(pg1, "JumpPower", "JP")

-- --- PAGE 2: FARM ---
Toggle(pg2, "Auto Instant Fish", "AutoFish"); Toggle(pg2, "Auto Tebak Kata", "AutoAnswer")

-- --- PAGE 3: TELEPORT (5 SLOT) ---
Input(pg3, "Target Name...", "TargetPlayer")
...
tpB.MouseButton1Click:Connect(function() pcall(function() local t = tostring(_G.FH4N_V490.TargetPlayer):lower(); for _, v in pairs(Players:GetPlayers()) do if v ~= Player and (v.Name:lower():find(t) or v.DisplayName:lower():find(t)) then Player.Character:PivotTo(v.Character:GetPivot()); break end end end) end)
...
for i = 1, 5 do
    ...
    s.MouseButton1Click:Connect(function() _G.FH4N_V490.SavedPos[i] = Player.Character:GetPivot(); s.Text = "SAVED!" task.wait(0.5) s.Text = "SAVE P"..i end)
    l.MouseButton1Click:Connect(function() if _G.FH4N_V490.SavedPos[i] then Player.Character:PivotTo(_G.FH4N_V490.SavedPos[i]) end end)
end

-- --- PAGE 4: SETTINGS & CONFIG ---
local function ColorGrid(p, title, key)
    local l = Instance.new("TextLabel", p); l.Size = UDim2.new(1, 0, 0, 20); l.Text = title; l.TextColor3 = Color3.new(1,1,1); l.BackgroundTransparency = 1; l.TextXAlignment = "Left"
    local g = Instance.new("Frame", p); g.Size = UDim2.new(1, -10, 0, 70); g.BackgroundTransparency = 1; Instance.new("UIGridLayout", g).CellSize = UDim2.new(0, 25, 0, 25)
    for _, col in pairs(Colors) do local c = Instance.new("TextButton", g); c.Text = ""; c.BackgroundColor3 = col; Instance.new("UICorner", c); c.MouseButton1Click:Connect(function() _G.FH4N_V490[key] = col end) end
end
ColorGrid(pg4, "STROKE COLOR", "StrokeColor"); ColorGrid(pg4, "BG COLOR", "BGColor")
Input(pg4, "BRIGHTNESS (0.5 - 10)", "Brightness")
...
for i = 1, 3 do
    ...
    local s = Instance.new("TextButton", f); s.Size = UDim2.new(0.48,0,1,0); s.Text = "SAVE CFG "..i; s.BackgroundColor3 = Color3.fromRGB(0, 50, 150); s.TextColor3 = Color3.new(1,1,1); Instance.new("UICorner",s)
    ...
    s.MouseButton1Click:Connect(function() _G.FH4N_V490.Configs[i] = {WS=_G.FH4N_V490.WS, JP=_G.FH4N_V490.JP, BC=_G.FH4N_V490.BGColor, SC=_G.FH4N_V490.StrokeColor, BR=_G.FH4N_V490.Brightness}; s.Text = "SAVED!" task.wait(0.5) s.Text = "SAVE CFG "..i end)
    l.MouseButton1Click:Connect(function() local d = _G.FH4N_V490.Configs[i] if d then _G.FH4N_V490.WS=d.WS; _G.FH4N_V490.JP=d.JP; _G.FH4N_V490.BGColor=d.BC; _G.FH4N_V490.StrokeColor=d.SC; _G.FH4N_V490.Brightness=d.BR; l.Text = "LOADED!" else l.Text = "EMPTY!" end task.wait(0.5) l.Text = "LOAD CFG "..i end)
end

-- --- ENGINES ---
local function CreateESP(p)
    if p.Character and not p.Character:FindFirstChild("FH4N_ESP") then
        local box = Instance.new("BoxHandleAdornment", p.Character)
        box.Name = "FH4N_ESP"; box.AlwaysOnTop = true; box.ZIndex = 5; box.Adornee = p.Character; box.Color3 = _G.FH4N_V490.StrokeColor; box.Size = Vector3.new(4, 5.5, 1); box.Transparency = 0.6
    end
end
